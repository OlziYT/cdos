          ...
          <div className="inline-block min-w-full py-2 align-middle md:px-6 lg:px-8">
            <div className={`overflow-hidden shadow ring-1 ${isDark ? 'ring-gray-700' : 'ring-black ring-opacity-5'} md:rounded-lg`}>
              <table className={`min-w-full divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-300'}`}>
                ...
                <tbody className={`divide-y ${isDark ? 'divide-gray-700' : 'divide-gray-200'} ${isDark ? 'bg-gray-900' : 'bg-white'}`}>
                  {clubs.map((club) => (
                    <tr key={club.id} className={isDark ? 'hover:bg-gray-800/50' : 'hover:bg-gray-50'}>
                      <td className={`whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium ${isDark ? 'text-gray-300' : 'text-gray-900'} sm:pl-6`}>
                        {club.name}
                      </td>
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <div className="flex justify-end gap-2">
                          ...
                          >
                            ...
                            <span className="sr-only">Modifier {club.name}</span>
                          </Link>
                          <button
                          ...
                          >
                            ...
                            <span className="sr-only">Supprimer {club.name}</span>
                          </button>
                        </div>
                      </td>
                    </tr>
